...
def fit_model(X, Y, bs, nb_epoch, student, teacher):
    ...
    checkpoint = ModelCheckpoint("directexpdeep", monitor='loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True)
    csv_logger = CSVLogger("log_directstudentexpdeep.csv", append=True, separator=';')
...
def biGRU_big(bs,time_steps,alphabet_size):
  ...
  x = Bidirectional(CuDNNGRU(64, stateful=False, return_sequences=True))(x)
  x = Bidirectional(CuDNNGRU(64, stateful=False, return_sequences=True))(x)
  x = Bidirectional(CuDNNGRU(64, stateful=False, return_sequences=True))(x)
