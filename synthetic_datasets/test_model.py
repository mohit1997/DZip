...
def fit_model(X, Y, bs, nb_epoch, student, teacher):
	...
	student.compile(loss=loss_fn, optimizer=optim, metrics=['acc'])
	checkpoint = ModelCheckpoint("modelstudent", monitor='loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True)
	csv_logger = CSVLogger("log_student.csv", append=True, separator=';')
	...
	student.fit_generator(train_gen, epochs=nb_epoch, verbose=1, validation_data=val_gen, callbacks=callbacks_list, use_multiprocessing=True, workers=0)
...
model_teacher = biGRU_big(batch_size, sequence_length, n_classes)
from keras.models import load_model
model_teacher.load_weights('directbigru')
optim = keras.optimizers.Adam(lr=1e-3, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
model_teacher.compile(loss=loss_fn, optimizer=optim, metrics=['acc'])

model_teacher.load_weights('directbigru')
gen = DataGenerator(X, Y, None, batch_size, n_classes, True, use_model=False)

print(model_teacher.evaluate_generator(gen, verbose=1))
