...
def fit_model(X, Y, bs, nb_epoch, student, teacher):
	...
	i = i+1
	out = student.test_on_batch(batch_x, [batch_y, batch_y])
        loss_list.append(out[1])
	out = student.train_on_batch(batch_x, [batch_y, batch_y])
	if i%10==0:
	    sys.stdout.flush()
	    print('Batch {}/{} Loss = {:4f} current batch Loss = {:4f}'.format(i, len(y)//bs, np.mean(loss_list), np.mean(loss_list[-10:])), end='\r')
            # print(out, end='\r')
        if i%100000==0:
            np.save('multitransfer_explosslist', np.array(loss_list))
...
def biGRU_big(bs,time_steps,alphabet_size):
  inputs_bits = Input(shape=(time_steps,))
  x = emb = Embedding(alphabet_size, 16,)(inputs_bits)
  ...
  x = Add(name='logits')([Dense(alphabet_size)(prelogits),  Dense(alphabet_size)(flat)])
  new_logits = Add(name='logits-new')([Dense(alphabet_size)(prelogits),  Dense(alphabet_size)(flat)])
  s1 = Activation('softmax', name="prev1")(x)
  s2 = Activation('softmax', name="prev2")(x)
